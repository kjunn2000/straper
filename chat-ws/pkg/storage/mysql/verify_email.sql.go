...
func (q *Queries) CreateVerifyEmailToken(ctx context.Context, token account.VerifyEmailToken) error {
	sql, args, err := sq.Insert("verify_email_token").Columns("token_id", "user_id").Values(token.TokenId, token.UserId).ToSql()
	if err != nil {
...
func (q *Queries) GetVerifyEmailToken(ctx context.Context, tokenId string) (account.VerifyEmailToken, error) {
	sql, args, err := sq.Select("token_id", "user_id").From("verify_email_token").Where(sq.Eq{"token_id": tokenId}).ToSql()
	if err != nil {
